...
template<std::size_t SZ>
std::istream &( operator>>)(std::istream &is, TFHE::CipherText<SZ> &text) {
    for (int i = 0; i < SZ; ++i)
...
template<std::size_t SZ>
std::ostream &( operator<<)(std::ostream &os, const TFHE::CipherText<SZ> &text) {
    for (int i = 0; i < SZ; ++i)
...
namespace TFHE {
    ...
    public:
        ...
        friend std::istream &(::operator>>)(std::istream &is, TFHE::CipherText<T> &text);

        template<std::size_t T>
        friend std::ostream &(::operator<<)(std::ostream &os, const TFHE::CipherText<T> &text);
    };
