    ...
    array<CipherText<32>, 8> hash(const vector <CipherText<8>> &_message, const CloudKey &key) {
        ...
        for (int i = 0; i < msg_len; i += 64) {
            ...
            for (int j = 0; j < 64; j += 4) {
                auto m0 = word_eval.left_shift(byteToWordEncryptText(msg[i + j]), 24), m1 = word_eval.left_shift(
                        byteToWordEncryptText(msg[i + j + 1]), 16)
                , m2 = word_eval.left_shift(byteToWordEncryptText(msg[i + j + 2]), 8), m3 = byteToWordEncryptText(
                        msg[i + j + 3]);
            ...
            for (int j = 16; j < 64; ++j) {
                ...
                const auto s0 = word_eval.xor_gate(right_rotate(w[j - 15], 7, word_eval),
                                                   word_eval.xor_gate(right_rotate(w[j - 15], 18, word_eval),
                                                                      word_eval.right_shift(w[j - 15], 3)));
                const auto s1 = word_eval.xor_gate(right_rotate(w[j - 2], 17, word_eval),
                                                   word_eval.xor_gate(right_rotate(w[j - 2], 19, word_eval),
                                                                      word_eval.right_shift(w[j - 2], 10)));
            ...
            for (int j = 0; j < 64; ++j) {
                const auto s0 = word_eval.xor_gate(right_rotate(a, 2, word_eval),
                                                   word_eval.xor_gate(right_rotate(a, 13, word_eval),
                                                                      right_rotate(a, 22, word_eval)));
                const auto maj = word_eval.xor_gate(word_eval.and_gate(a, b),
                                                    word_eval.xor_gate(word_eval.and_gate(a, c),
                                                                       word_eval.and_gate(b, c)));
                const auto t2 = word_eval.add(s0, maj);
                const auto s1 = word_eval.xor_gate(right_rotate(e, 6, word_eval),
                                                   word_eval.xor_gate(right_rotate(e, 11, word_eval),
                                                                      right_rotate(e, 25, word_eval)));
                const auto ch = word_eval.xor_gate(word_eval.and_gate(e, f),
                                                   word_eval.and_gate(word_eval.not_gate(e), g));
