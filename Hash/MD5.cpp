...
namespace MD5 {
		...
		template<size_t Sz> inline static CipherText<Sz> left_rotate(const CipherText<Sz> &num, size_t offset, const Evaluator<Sz> &eval) {
		...
		static void swapByteOrder(CipherText<32> &cip, const Evaluator<32> &word_eval) {
			cip = word_eval.or_gate(word_eval.right_shift(cip, 24)
							, word_eval.or_gate(word_eval.and_gate(word_eval.left_shift(cip, 8), word_eval.constant(0x00FF0000))
		...
		array<CipherText<32>, 4> hash(const vector<CipherText<8>> &_message, const CloudKey &key) {
			...
			for(int i = 0; i < msg_len; i += 64) {
				...
				for(int j = 0; j  < 64; j += 4) {
					auto m0 = byteToWordEncryptText(msg[i + j]), m1 = word_eval.left_shift(byteToWordEncryptText(msg[i + j + 1]), 8)
									, m2 = word_eval.left_shift(byteToWordEncryptText(msg[i + j + 2]), 16), m3 = word_eval.left_shift(byteToWordEncryptText(msg[i + j + 3]), 24);
